...
export default {
  ...
  setup() {
    ...
    const submitForm = async () => { const valid = await doctorFormRef.value.validate(); if (!valid) return; await createDoctor(doctorForm.value); ElMessage.success('??????'); router.push('/doctor') }
